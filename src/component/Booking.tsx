...
const [paymentMethod, setPaymentMethod] = useState('card'); 
  ...
  return (
    <motion.div 
        ...
        className="max-w-6xl mx-auto px-4 py-10 bg-[var(--color-mind-bg)] min-h-screen"
    >
      <motion.h2 variants={itemVariants} className="text-3xl font-extrabold text-center mb-8 text-gray-900">
      ...
      {isSuccess ? (
         <motion.div 
            ...
            exit={{ opacity: 0 }}
            className="bg-green-50 p-8 rounded-xl text-center border border-green-200 shadow-lg max-w-md mx-auto mt-10"
         >
            <motion.div 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 space-y-6">
                ...
                <motion.div variants={itemVariants} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {PROFESSIONALS.map(p => (
                            ...
                            className={`p-3 rounded-xl border cursor-pointer transition-colors flex flex-col items-center text-center gap-2 ${selectedProfessional?.id === p.id ? 'bg-teal-50 border-[var(--color-primary-teal)] ring-1 ring-[var(--color-primary-teal)]' : 'border-gray-100 hover:border-[var(--color-primary-teal)] hover:shadow-md'}`}
                            >
                ...
                <motion.div 
                    ...
                    animate={{ 
                        opacity: selectedProfessional ? 1 : 0.5, 
                        // filter: selectedProfessional ? 'blur(0px)' : 'blur(1px)',
                        pointerEvents: selectedProfessional ? 'auto' : 'none'
                    }}
            ...
            <div className="lg:col-span-4 space-y-6">
                ...
                {/* Summary Card */}
                <motion.div variants={itemVariants} className="bg-white p-5 rounded-2xl shadow-lg border border-gray-100">
                    ...
                    <div className="space-y-3 text-sm">
                        ...
                        {/* Animated Total Price Update */}
                        <div className="border-t border-dashed border-gray-200 pt-3 mt-2 flex justify-between items-center">
                            <span className="font-bold text-gray-900">Total Due</span>
                            <motion.span 
                                key={sessionType.price} // Triggers animation on change
                                initial={{ scale: 1.2, color: '#3B82F6' }}
                                animate={{ scale: 1, color: '#0D9488' }} // Teal color
                                className="font-extrabold text-2xl text-[var(--color-primary-teal)]"
                            >
                                ${sessionType.price}
                            </motion.span>
                        </div>
